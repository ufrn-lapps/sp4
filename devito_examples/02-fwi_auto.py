...
def fwi_gradient(vp_in):
    ...
    for i in range(9):
        ...
        op_fwd.apply(src=src_true, rec=rec_true, u=u0_man,
...
# ff, update,ff_man,update_man = fwi_gradient(model0.m)
...
for i in range(0, fwi_iterations):
    _, _, phi, direction = fwi_gradient(model0.vp)
    history_man[i] = phi
...
print('AUTO')
for i in range(0, fwi_iterations):
    phi, direction, _, _ = fwi_gradient(model0.vp)
    history[i] = phi
    alpha = .05 / np.abs(direction).max()
    model0.vp = apply_box_constraint(model0.vp.data - alpha * direction)
    print('Objective value is %f at iteration %d' % (phi, i+1))
