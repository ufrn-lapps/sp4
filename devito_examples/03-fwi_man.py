...
def apply_box_constraint(vp):
    """Restrict data into the desired interval. 2.0 to 3.5.
    """
    logging.info("Applying constraint box")
...
def fwi_gradient(vp_in):
    ...
    for i in range(9):
        ...
        # receiver
        rec_true = Receiver(name='rec', grid=model.grid, time_range=time_axis,
...
if __name__ == "__main__":
    ...
    logger = logging.getLogger("FWI MANUAL")
    ...
    logger.info("spacing = %s" % str(spacing))
    ...
    logger.info("Initial velocity model vp=%f", 2.5)
    ...
    rec_coordinates[:, 1] = np.linspace(
        0, model.domain_size[0], num=nreceivers)
