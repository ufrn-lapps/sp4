...
def experiment_subdomain(water_level=0.5):
    ...
    vp[:,:,:int(water_level*shape[2])+1] = 1.52
    vs[:,:,:int(water_level*shape[2])+1] = 0.
    rho[:,:,:int(water_level*shape[2])+1] = 1.05

    vp[:,:,int(water_level*shape[2])+1:int(water_level*shape[2])+1+int(4/h)] = 1.6
    vs[:,:,int(water_level*shape[2])+1:int(water_level*shape[2])+1+int(4/h)] = 0.4
    rho[:,:,int(water_level*shape[2])+1:int(water_level*shape[2])+1+int(4/h)] = 1.3

    vp[:,:,int(water_level*shape[2])+1+int(4/h):] = 2.2
    vs[:,:,int(water_level*shape[2])+1+int(4/h):] = 1.2
    rho[:,:,int(water_level*shape[2])+1+int(4/h):] = 2.
    ...
    t0, tn = 0., 30.
    dt = np.float32(0.9*model.critical_dt)
...
for i in range(1):
    for water_level in np.arange(0.1, 0.9, 0.1):
        ...
        experiment_subdomain(water_level)
